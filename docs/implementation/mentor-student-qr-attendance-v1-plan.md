...
- `GET /api/admin/records`
  ...
  - accepts optional `startDate` and `endDate` query params for an inclusive stored `event_date` range
  - exposes the resolved range in `dateFilter`
...
- `GET /api/admin/export.csv`
  ...
  - uses the same inclusive `startDate` / `endDate` range contract as the records table
...
- surface visible start/end date controls with an apply action for reporting
- support note edits
...
- build records table view with start/end date filtering
...
- admin records/export honor the inclusive `startDate` / `endDate` range contract and event-day fallback
